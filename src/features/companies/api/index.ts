...
const companiesApi = {
  ...
  createCompany: async (data: CreateCompanyData): Promise<Company> => {
    ...
    Object.entries(data).forEach(([key, value]) => {
      ...
      } else if (key === 'idImage' && value instanceof File) {
  ...
  return useMutation({
    ...
    onMutate: async (updatedCompany) => {
      ...
      await queryClient.cancelQueries({
        queryKey: queryKeys.companies.detail(updatedCompany._id),
      })
      ...
      queryClient.setQueriesData(
        { queryKey: queryKeys.companies.lists() },
        (old: { companies?: Company[] } | undefined) => {
    ...
    onSettled: (data) => {
      ...
      if (data) {
        queryClient.invalidateQueries({
          queryKey: queryKeys.companies.detail(data._id),
        })
      }
