...
import type { Property, PropertiesResponse, PropertyFilters, CreatePropertyFormData, UpdatePropertyFormData } from '../types'
...
export const propertiesApi = {
  ...
  getProperties: async (filters?: PropertyFilters): Promise<PropertiesResponse> => {
    ...
    const response = await apiClient.post<PropertiesResponse>('/api/property/admin/search', requestBody)
    return response
  },
  ...
  // Update property
  updateProperty: async ({ _id, ...data }: UpdatePropertyFormData): Promise<Property> => {
    const formData = new FormData()
    
    // Add text fields
    if (data.shortAddress !== undefined) formData.append('shortAddress', data.shortAddress || '')
    ...
    if (data.isManaged !== undefined) formData.append('isManaged', data.isManaged.toString())
    if (data.redirectUrl !== undefined) formData.append('redirectUrl', data.redirectUrl || '')
    ...
    const response = await apiClient.put<Property>(`/api/property/${_id}`, formData, {
      headers: {
        'Content-Type': 'multipart/form-data'
      }
    })
  ...
  updatePropertyStatus: async (id: string, status: string): Promise<Property> => {
    const response = await apiClient.patch<Property>(`/api/property/status/${id}/${status}`, {})
    return response
  },
  ...
  archiveProperty: async (id: string): Promise<Property> => {
    const response = await apiClient.post<Property>(`/api/property/${id}/archive`)
    return response
  },

  // Restore property
  restoreProperty: async (id: string): Promise<Property> => {
    const response = await apiClient.post<Property>(`/api/property/${id}/restore`)
    return response
  },

  // Duplicate property
  duplicateProperty: async (id: string): Promise<Property> => {
    const response = await apiClient.post<Property>(`/api/property/${id}/duplicate`)
    return response
  },

  // Assign property to user
  assignProperty: async (id: string, userId: string): Promise<Property> => {
    const response = await apiClient.post<Property>(`/api/property/${id}/assign`, { userId })
    return response
  },

  // Update property management status
  updateManagement: async (id: string, isManaged: boolean): Promise<Property> => {
    const response = await apiClient.patch<Property>(`/api/property/update-management/${id}`, { isManaged })
    return response
  }
}
...
export const useUpdateManagement = () => {
  ...
  return useMutation({
    ...
    onError: (error) => {
      const message = error instanceof AxiosError 
        ? error.response?.data?.message || 'Failed to update management status'
